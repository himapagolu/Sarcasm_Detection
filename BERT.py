...
class HierarchicalBERTWithPEFT(nn.Module):
    def __init__(self, model_name='bert-base-uncased', num_classes=2, lstm_hidden=128):
        super().__init__()
        ...
        self.context_bert = get_peft_model(BertModel.from_pretrained(model_name), peft_config)
        self.response_bert = get_peft_model(BertModel.from_pretrained(model_name), peft_config)
    ...
    def forward(self, context_input_ids, context_attention_mask, response_input_ids, response_attention_mask):
        ...
        for i in range(num_utterances):
            output = self.context_bert(input_ids=context_input_ids[:, i, :], attention_mask=context_attention_mask[:, i, :]).last_hidden_state[:, 0, :]
            context_embs.append(output)

        context_stack = torch.stack(context_embs, dim=1).unsqueeze(1)
        cnn_out = torch.relu(self.context_cnn(context_stack)).squeeze(-1)
        cnn_out = cnn_out.permute(0, 2, 1)
        _, (h_n, _) = self.context_lstm(cnn_out)
        context_repr = torch.cat((h_n[-2], h_n[-1]), dim=1)

        response_output = self.response_bert(input_ids=response_input_ids, attention_mask=response_attention_mask).last_hidden_state
        _, (h_resp, _) = self.response_lstm(response_output)
        response_repr = torch.cat((h_resp[-2], h_resp[-1]), dim=1)
...
if __name__ == '__main__':
    print("\n Hierarchical BERT with PEFT initialized successfully.")
    ...
    model = HierarchicalBERTWithPEFT().to(device)
    ...
    try:
        _, test_acc, test_f1 = evaluate(model, test_loader, criterion)
        print(f"Test Accuracy: {test_acc:.4f} | Test F1: {test_f1:.4f}")
        torch.save(model.state_dict(), "hierarchical_bert_sarcasm_model.pt")
